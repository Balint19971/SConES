...
const useFetchMyPaper = (user: string) => {
  ...
  useEffect(() => {
    const fetchPaper = async () => {
      const url = `http://${process.env.REACT_APP_BACKEND_HOST}:${process.env.REACT_APP_BACKEND_PORT}/api/papers/${user}`;
...
export default useFetchMyPaper;
